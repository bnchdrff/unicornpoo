...
if (poosites.hasOwnProperty(poositename)) {
  ...
  var content_left = 0;
  window.find(content_el);
  if (content_el.offsetParent) {
    do {
      content_left += content_el.offsetLeft;
    } while (content_el = content_el.offsetParent);
  }
  window.console.log(content_left);
  ...
  if (poosites[poositename].hasOwnProperty('search')) {
    ...
    addEvent(document.getElementById('shared-header-search-q'), 'focus', function(ev) {
      $search-chooser.innerHTML = '<span style="font-size:.8em;text-transform: uppercase;">Search all AMP sites with Google</a>';
      $searchChooser.style.visibility = 'visible';
    });
  }
}
